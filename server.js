...
app.post('/api/biblioteca', (req, res) => {
  const { nome, vida, classe, nivel, ataque, defesa, ativo, dataDeEntrada } = req.body;
  if (!nome) { return res.status(400).json({ error: 'O nome é obrigatório' }); }
  const sql = `INSERT INTO Biblioteca ( nome, vida, classe, nivel, ataque, defesa, ativo, dataDeEntrada) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
  const params = [ nome, vida, classe, nivel, ataque, defesa, ativo, dataDeEntrada ];
...
app.put('/api/biblioteca/:id', (req, res) => {
  const { nome, vida, classe, nivel, ataque, defesa, ativo, dataDeEntrada } = req.body;
  const sql = `UPDATE Biblioteca SET nome = ?, vida = ?, classe = ?, nivel = ?, ataque = ?, defesa = ?, ativo = ?, dataDeEntrada = ?, WHERE id = ?`;
  const params = [ nome, vida, classe, nivel, ataque, defesa, ativo, dataDeEntrada, req.params.id ];
